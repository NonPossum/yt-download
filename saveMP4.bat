...
:: Uruchomienie yt-dlp z zapisem do folderu yt w formacie mp4
yt-dlp -a "%inputFile%" --download-archive "%archiveFile%" -P "%outputFolder%" -f bestvideo+bestaudio --merge-output-format "%videoFormat%" 2> error.log
...
echo Pobieranie zakonczone sukcesem! Pliki zapisane w folderze %outputFolder% w formacie %videoFormat%.
pause
